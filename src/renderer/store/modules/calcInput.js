class Btn {
  constructor (obj) {
    ...
    this.type = obj.type // il tipo di pulsante (number, operator, clear, equal)
  }
...
const BTN_DIVIDE = new Btn({key: 'DIVIDE', label: '÷', icon: 'mdi-division', value: 'DIVIDE', type: 'operator'})
const BTN_MOLT = new Btn({key: 'MOLT', label: '*', icon: 'mdi-multiplication', value: 'MOLTIPLICA', type: 'operator'})
const BTN_MINUS = new Btn({key: 'MINUS', label: '-', value: 'SOTTRAE', type: 'operator'})
const BTN_PLUS = new Btn({key: 'PLUS', label: '+', icon: 'mdi-plus', value: 'SOMMA', type: 'operator'})
const BTN_SIGN = new Btn({key: 'SIGN', label: '±', value: 'INVERTI', type: 'operator'})
const BTN_OP_PERC = new Btn({key: 'OP_PERC', label: '%', icon: 'mdi-percent', value: 'PERCENTUALE', type: 'operator'})
const BTN_OP_SQUARE = new Btn({key: 'OP_SQUARE', label: '√', icon: 'mdi-square-root', value: 'RADICE', type: 'operator'})
const BTN_OP_POT = new Btn({key: 'OP_POT', label: 'x²', icon: 'mdi-format-superscript', value: 'ELEVA', symbol: '²', type: 'operator'})
const BTN_OP_1DIV = new Btn({key: 'OP_1DIV', label: '⅟', value: '1DIV', type: 'operator'})
const BTN_POINT = new Btn({key: 'POINT', label: ',', value: 'DECIMALE', type: 'operator'})
